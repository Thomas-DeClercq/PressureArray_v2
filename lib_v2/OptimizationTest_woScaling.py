...
def PressureDistribution(X,p0,std,lx,ly,r_curve,theta,x0,y0):
    ...
    t0 = time.time()
    X_new,Y_new,Z_new = allInOne_MeshUnknown(p0,std,lx,ly,r_curve,theta,x0,y0)
    t1 = time.time()
...
def Optimization(xi,yi,pressureArray, shape, spacing, t=6,n=9):
    ...
    E = curve_fit(PressureDistribution, xdata, ydata, x0, bounds=(lb, ub), method = meth[0], maxfev=100,verbose=2,ftol=1e-4,diff_step=1,x_scale=x_scale)#,diff_step=100)
    #print(E)
    ### denormalize
