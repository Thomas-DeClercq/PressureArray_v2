...
def Optimization(xi,yi,pressureArray, shape, spacing, Z_orginial, t=6,n=9,x0=None, it_max = 10, ):
    """
    ...
    Zi = np.zeros((amount,))
    pressureArray_Zi = pressureArray - Zi
    
    while (max(abs(pressureArray_Zi)/max(pressureArray)) > 0.33) and (it < it_max):
        array = np.zeros((amount,3)) # n points with highest pressure; 2: pressure; 3: x coordinate; 4: y coordinate
        
        order = np.argsort(pressureArray_Zi)
        order = np.flip(order)
        
        array.T[0] = pressureArray_Zi[order]
        array.T[1] = xi[order]
        array.T[2] = yi[order]
        print("p_error_max: "+str(max(abs(pressureArray_Zi)/max(pressureArray))))
        print(array)
        """
        ...
        Zi = calc_Z(xi, yi, *E.x)
        pressureArray_Zi = pressureArray_Zi - Zi
        it += 1
...
if __name__ == '__main__':
    ...
    for ind in range(10):#len(list_files)):
        print("------------------------------------------------")
        print(ind+1)
        
        Z_all_real = np.zeros((50,50))
        Z_all_recon = np.zeros((50,50))
        ...
        for i in range(3):
            ...
            if i == 0:
                ...
                params = [10, 1, 12, 0.5, 0, 90, 4.5, 35.2]
            elif i == 2:
                params = [10, 1, 8, 0.5, 1.4, -45, 5, 23]
        ...
        array, list_E = Optimization(xi,yi,Z_i.copy(),shape,spacing, Z_all_real, n=45,it_max=5)
        """
        ...
        RMSE_i = compare_Z(shape,spacing,X,Y,Z_all_real,Z_all_recon)
