...
def pressureReconstruction_df(df,shape,spacing,n=50,m=10,p_m=0.75,iterations=5):
    ...
    df_new = np.zeros((timesteps*it_out,13))
    ...
    for t in range(timesteps):
        ...
        for idx1 in range(it_out):
            ...
            df_new[t*it_out+idx1,10] = df.loc[df['Timestep']==t].values[-1][-3]
            df_new[t*it_out+idx1,11] = df.loc[df['Timestep']==t].values[-1][-2]
            df_new[t*it_out+idx1,12] = df.loc[df['Timestep']==t].values[-1][-1]
            for idx2 in range(8):
...
if __name__ == "__main__":
    ...
    training_data = 'TrainingData/trans_rot'
    ...
    #all_training_files = all_training_files[100000:] #select only_rot[:50000] / trans[50000:100000] / trans_rot[100000:]
    random.shuffle(all_training_files) #only rot
    training_files = all_training_files[:training_size]

    test_files = os.listdir(f"{dir_data}/{test_data}")
    #test_files.sort()
    #test_files = test_files[2000:] #select only_rot [:1000] / trans[1000:2000]  / trans_rot[2000:]
    ...
    for idx,file in enumerate(training_files):
        ...
        for t in range(0,amount_of_timesteps+1):
            ...
            try:
                ...
                y_train_i = df_t.loc[df["Timestep"]==current_timestep][['Dx','Dy','D_angle']].values[0] #- df_t.loc[df["Timestep"]==(current_timestep-1)]['angle'].values[0]
    ...
    for idx,file in enumerate(test_files):
        ...
        for t in range(0,amount_of_timesteps+1):
            ...
            try:
                ...
                y_test_i = df_t.loc[df["Timestep"]==current_timestep][['Dx','Dy','D_angle']].values[0] #- df_t.loc[df["Timestep"]==(current_timestep-1)]['angle'].values[0]
                #angles = df_t['angle'].values[-1] - df_t['angle'].values[-2]
    ...
    error_array_angle = y_test.T[2]
    error_array_angle = np.vstack((error_array_angle,y_pred.T[2].flatten()))
    ...
    print(f"average dist: {np.average(error_array[2])}")
    print(f"median dist: {np.median(error_array[2])}")
    print(f"std dist: {np.std(error_array[2])}")
    ...
    if save:
        dnn_model.save(f'./NNs/trans_rot/NN_{PR_it}it_{int(training_size/1000)}k')
    ...
    if plot:
        ...
        ax24 = plt.subplot(514)
        ax24.plot(error_array_angle[0],error_array_angle[1],'.')
        ax24.set_xlabel('real')
        ax24.set_ylabel('estimated')
        ax24.plot([-20,90],[-20,90],color='k')
        ax24.set_title('angle error')
        ax25 = plt.subplot(515)
        ax25.boxplot([error_array_rel_x[2],error_array_rel_y[2],error_array[2],error_array_angle[2]], showfliers=False)
        plt.show(block=False)
