...
from ValidateFromFEM_roundedSurface_v2 import GaussianPressureDistribution, compare2FEM, calcForce_int #roundedSurface_array
...
def calcForce_roundedSurface(par):
    ...
    if int_type == 1:        
        ...
        F_sides = 0.5*P*p0*sqrt(2*pi)*std*10**-6
...
for ind in range(len(list_files)):
    ...
    for ti in range(1):
        ...
        #print(E)
        F_int = calcForce_int(E.x,shape,spacing,n=10)
        dur = time.time()-t0
        ...
        print("F_int: "+str(F_int))
        F_ana = calcForce_roundedSurface(E.x)
        print("F_ana :"+str(F_ana))
        F_ref = data_raw[46]
        #print("reference force: "+str(F_ref))
        print("force error: "+str(F_ref-F_int))
        
        
        #list_error_i.append(sqrt(np.square(errorMatrix).mean()))
        list_error_i.append((F_ref-F_int)/F_int)
