...
def roundedSurface_CF_3(X,p0,std,theta1,lx,ly,x0,y0,ttype):
    int_type = round(ttype)
    ...
    #print(Z)
    if int_type == 1: #rectangle-Guassian
...
def GaussianPressureDistribution(pressureArray, shape, spacing, t=6,n=9):
    ...
    # x0 = [p0, std, theta, lx, ly, x0, y0, type]
    ...
    """
    x0 = [array_n[0][1],2,0,0,0,(array_n.T[2][nx[0]]+array_n.T[2][nx[1]])/2,(array_n.T[3][ny[0]]+array_n.T[3][ny[1]])/2,typee]
    #print(x0)
    ...
    lb = [x0[0], 0, -90, 0, 0, min(array_n.T[2][nx[0]],array_n.T[2][nx[1]]), min(array_n.T[3][ny[0]],array_n.T[3][ny[1]]),1]
    ub = [x0[0]*10, 10, 90, spacing*max(shape), spacing*max(shape), max(array_n.T[2][nx[0]],array_n.T[2][nx[1]]), max(array_n.T[3][ny[0]],array_n.T[3][ny[1]]),2]
    ...
    E = curve_fit(roundedSurface_CF_3, xdata, ydata, x0, bounds=(lb, ub), method = meth[0], maxfev=1000, verbose=2, ftol=1e-2)
    #print(E)
    ### denormalize
...
for ind in range(len(list_files)):
    ...
    #data_raw = data_raw.T[random.randint(2,len(data_raw[0])-1)]
        ...
        E = GaussianPressureDistribution(pres.copy(),shape,spacing,n=30)[1][0]
        dur = time.time()-t0
