...
cdef goodGuess(np.ndarray[DTYPE_t,ndim=2] array, list shape, float spacing, np.ndarray[DTYPE_t,ndim=2] list_x0):
    ...
    cdef list lb = [min(0.5*p_x0,3*p_x0), 1, 0, 0, 0, 0, min(array_08.T[1]), min(array_08.T[2])] #[x0[0], 0, 0, 0, -100, -90, min(array_n.T[1][nx[0]],array_n.T[1][nx[1]]), min(array_n.T[2][ny[0]],array_n.T[2][ny[1]])] #[x0[0], 0, 0, 0, 0.1, 0.1, 0, -20, 1, -90, min(array_n.T[1][nx[0]],array_n.T[1][nx[1]]), min(array_n.T[2][ny[0]],array_n.T[2][ny[1]])]
    ...
    cdef list ub = [max(0.5*p_x0,3*p_x0), 10, spacing*max(shape), spacing*max(shape), 4, 90,  max(array_08.T[1]), max(array_08.T[2])]
    
    
    cdef list x0_calc = [p_x0, x0_sigma, x0_lx, x0_ly, x0_theta, 0, x0_x, x0_y]
