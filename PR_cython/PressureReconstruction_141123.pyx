...
cdef goodGuess(np.ndarray[DTYPE_t,ndim=2] array, list shape, float spacing, np.ndarray[DTYPE_t,ndim=2] list_x0):
    ...
    p_n0 = 0.5
    cdef unsigned int len_array_0 = len(array_0)
    
    for i0 in range(len_array_0):
        if array_0[i0][0] > p_n0:
            n0 += 1
    
    if n0 < 4:
        n0 = 4
    cdef np.ndarray[DTYPE_t,ndim=2] array_08

    if neg:
        array_08 = array_0[(len_array_0-n0):]
    else:
        array_08 = array_0[:n0]
    #print(array_08)
    ...
    cdef list lb = [min(0.75*p_x0,1.25*p_x0), 1, 0, 0, 0, -180, min(array_08.T[1]), min(array_08.T[2])] #[x0[0], 0, 0, 0, -100, -90, min(array_n.T[1][nx[0]],array_n.T[1][nx[1]]), min(array_n.T[2][ny[0]],array_n.T[2][ny[1]])] #[x0[0], 0, 0, 0, 0.1, 0.1, 0, -20, 1, -90, min(array_n.T[1][nx[0]],array_n.T[1][nx[1]]), min(array_n.T[2][ny[0]],array_n.T[2][ny[1]])]
    ...
    cdef list ub = [max(0.75*p_x0,1.25*p_x0), 10, spacing*max(shape), spacing*max(shape), 4, 180,  max(array_08.T[1]), max(array_08.T[2])]
    
    
    cdef list x0_calc = [p_x0, x0_sigma, min(x0_lx+spacing, spacing*max(shape)-0.0001), min(x0_ly, spacing*max(shape)-0.0001), 0.5, x0_theta, x0_x, x0_y]
