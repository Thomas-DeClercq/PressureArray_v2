...
cdef find_best_x0(np.ndarray[DTYPE_t,ndim=2] list_x0, list lb, list ub, float s):
    ...
    for it in range(len(list_x0)):
        if (list_x0[it][0] > lb[0]) and (list_x0[it][0] < ub[0]) and (list_x0[it][5] > lb[5]) and (list_x0[it][5] < ub[5]) and(list_x0[it][6] > lb[6]-s) and (list_x0[it][6] < ub[6]+s) and (list_x0[it][7] > lb[7]-s) and (list_x0[it][7] < ub[7]+s):
            #check if near
...
cdef goodGuess(np.ndarray[DTYPE_t,ndim=2] array, list shape, float spacing, np.ndarray[DTYPE_t,ndim=2] list_x0):
    ...
    cdef float x0_sigma = 3

    cdef float x0_lx, dist, 
    cdef float max_dist = 0
    cdef int idx_i = 0
    cdef int idx_j = 0
    for i in range(n0):
        for j in range(i,n0):
            dist = (array_08[i][1]-array_08[j][1])**2 + (array_08[i][2]-array_08[j][2])**2
            if dist > max_dist:
                max_dist = dist
                idx_i = i
                idx_j = j
    x0_lx = sqrt(max_dist)
    cdef float x0_theta = atan2(array_08[idx_i][2]-array_08[idx_j][2],array_08[idx_i][1]-array_08[idx_j][1])/pi*180
    
    if x0_theta < 0:
        x0_theta = x0_theta+180
        
    cdef float d_theta = 15

    cdef float x0_ly, max_dist_x, max_dist_y
    max_dist_x = max(array_08.T[1]) - min(array_08.T[1])
    max_dist_y = max(array_08.T[2]) - min(array_08.T[2])
    x0_ly = min(max_dist_x, max_dist_y)

    cdef list lb = [min(0.5*p_x0,3*p_x0), 1, 0, 0, 0, max(0,x0_theta-d_theta), min(array_08.T[1]), min(array_08.T[2])] #[x0[0], 0, 0, 0, -100, -90, min(array_n.T[1][nx[0]],array_n.T[1][nx[1]]), min(array_n.T[2][ny[0]],array_n.T[2][ny[1]])] #[x0[0], 0, 0, 0, 0.1, 0.1, 0, -20, 1, -90, min(array_n.T[1][nx[0]],array_n.T[1][nx[1]]), min(array_n.T[2][ny[0]],array_n.T[2][ny[1]])]
    #cdef list lb = [0, 1, 0, 0, 0, -180, min(array_08.T[1]), min(array_08.T[2])] #[x0[0], 0, 0, 0, -100, -90, min(array_n.T[1][nx[0]],array_n.T[1][nx[1]]), min(array_n.T[2][ny[0]],array_n.T[2][ny[1]])] #[x0[0], 0, 0, 0, 0.1, 0.1, 0, -20, 1, -90, min(array_n.T[1][nx[0]],array_n.T[1][nx[1]]), min(array_n.T[2][ny[0]],array_n.T[2][ny[1]])]
    cdef list ub = [max(0.5*p_x0,3*p_x0), 10, spacing*max(shape), spacing*max(shape), 4, min(180,x0_theta+d_theta),  max(array_08.T[1]), max(array_08.T[2])]
    
    
    cdef list x0_calc = [p_x0, x0_sigma, x0_lx, x0_ly, 0, x0_theta, x0_x, x0_y]
...
def Optimization(np.ndarray[DTYPE_t,ndim=1] xi, np.ndarray[DTYPE_t,ndim=1] yi, np.ndarray[DTYPE_t,ndim=1] pressureArray, list shape, float spacing,  np.ndarray[DTYPE_t,ndim=2] array_rbf, np.ndarray[DTYPE_t,ndim=2] list_x0,int n=9, int it_max = 10, int t_max = 40):
    ...
    while ((max(abs(array.T[0]))/max(pressureArray)) > 0.05) and (it < it_max) and (time_opt+t_est < t_max):
    #while ((max(array.T[0])/max(pressureArray)) > 0.05) and (it < it_max) and (time_opt+t_est < t_max):
    #while it < it_max:    
    ...
    """
    if (time_opt+t_est > t_max):
        print("time limit reached. "+str(it)+" iteration done")
    elif (max(abs(array.T[0])/max(pressureArray)) < 0.05):
