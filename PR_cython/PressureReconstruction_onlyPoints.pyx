...
cdef goodGuess(np.ndarray[DTYPE_t,ndim=2] array, list shape, float spacing, np.ndarray[DTYPE_t,ndim=2] list_x0):
    ...
    cdef float p_x0 = array[0][0]
    
    cdef float x0_x = array[0][1] #np.mean(array_08.T[1])
    cdef float x0_y = array[0][2] #np.mean(array_08.T[2])

    cdef list lb = [0.75*p_x0, 0.1, 0, 0, 0, 0, x0_x-spacing, x0_y-spacing] #[x0[0], 0, 0, 0, -100, -90, min(array_n.T[1][nx[0]],array_n.T[1][nx[1]]), min(array_n.T[2][ny[0]],array_n.T[2][ny[1]])] #[x0[0], 0, 0, 0, 0.1, 0.1, 0, -20, 1, -90, min(array_n.T[1][nx[0]],array_n.T[1][nx[1]]), min(array_n.T[2][ny[0]],array_n.T[2][ny[1]])]
    ...
    cdef list ub = [1.25*p_x0, 2, 0.1, 0.1, 0.01, 0.01,  x0_x+spacing, x0_y+spacing]
