...
def GaussianPressureDistribution(pressureArray, shape, spacing, t=6,n=9):
    ...
    opt_2 = {'ftol':1e-7, 'xtol':1e-7,'maxfev':1000}
    opt_4 = {'ftol':1e-12,'maxiter':2*10**4}
    
    #time0 = time.time()
    #E = minimize(ObjFun,x0,args=(array_n,n_p),method = meth[2],bounds=bnds,options=opt_2)
    #E = minimize_parallel(fun=ObjFun, x0=x0, args=(array_n,n_p),bounds=bnds,options=opt_1)
    #E = differential_evolution(ObjFun,bnds,args=(array_n,n_p),workers=-1)
    E = dual_annealing(ObjFun,bnds,args=(array_n,n_p),x0=x0)
    #print('time - opt: '+str(time.time()-time0))
