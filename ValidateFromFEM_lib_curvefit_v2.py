...
def compare2FEM(shape,spacing,par,name_file,type_contact,plot=True):
    XX = np.arange(0, (shape[0]+1)*spacing, 0.1)
    YY = np.arange(0, (shape[1]+1)*spacing, 0.1)
    ...
    F_calc_int = scipy.integrate.simps([scipy.integrate.simps(zz_x,XX) for zz_x in Z_calc*10**-6],YY)
    print("F_calc_int: "+str(F_calc_int))
    ...
    F_meas_int = scipy.integrate.simps([scipy.integrate.simps(zz_x,XX) for zz_x in Z_meas*10**-6],YY)
    print("F_meas_int: "+str(F_meas_int))
...
def calcForce_roundedSurface(par):
    ...
    if int_type == 1:        
        ...
        F_sides = P*p0*0.5*sqrt(2*pi)*std*10**-6
        print(F_sides)
...
def GaussianPressureDistribution(pressureArray, shape, spacing, t=6,n=9):
    ...
    lb = [array_n[0][1], 0, -45, 0, 0, min(array_n.T[2][nx[0]],array_n.T[2][nx[1]]), min(array_n.T[3][ny[0]],array_n.T[3][ny[1]])]
    ub = [array_n[0][1]*10, 10, 45, spacing*max(shape), spacing*max(shape), max(array_n.T[2][nx[0]],array_n.T[2][nx[1]]), max(array_n.T[3][ny[0]],array_n.T[3][ny[1]])]
    ...
    #E_rect = list(curve_fit(roundedSurface_CF_rect, xdata, ydata, x0, bounds=(lb, ub), method = meth[0], maxfev=50, verbose=0, ftol=1e-2)[0])
    #cost_rect = calc_cost(E_rect,xdata,ydata,1)
    E_ellips = list(curve_fit(roundedSurface_CF_ellips, xdata, ydata, x0, bounds=(lb, ub), method = meth[0], maxfev=50, verbose=0, ftol=1e-2)[0])
...
if __name__ == "__main__"
    ...
    for ind in range(len(list_files)):
        ...
        #data_raw = data_raw.T[random.randint(2,len(data_raw[0])-1)]
            ...
            E = GaussianPressureDistribution(pres.copy(),shape,spacing,n=9)[1]
            dur = time.time()-t0
