...
def roundedSurface_array_backup(X,Y,p0,std,theta,lx,ly,x0,y0):
    Z = np.zeros(X.shape)
    theta = theta/180*pi
...
cdef roundedSurface_array(np.ndarray[DTYPE_t,ndim=2] X,np.ndarray[DTYPE_t,ndim=2] Y, float p0, float std, float theta1, float lx, float ly, float x0, float y0):
...
def GaussianPressureDistribution(pressureArray, shape, spacing, t=6, n=30, x0=None, ftol=1e-3, max_it= 10, verbose_bool=False):
    ...
    ub = [x0[0]*10, 10, 90, spacing*(max(shape)+1), spacing*(max(shape)+1), max(array_n.T[2][nx[0]],array_n.T[2][nx[1]]), max(array_n.T[3][ny[0]],array_n.T[3][ny[1]])]
    ...
    E = curve_fit(roundedSurface_CF, xdata, ydata, x0, bounds=(lb, ub), method = meth[0], maxfev=max_it, verbose=verbose,ftol=ftol)
