...
if __name__ == "__main__":
    ...
    for file in training_files:
        ...
        max_iterations = int(df["iteration"].max()+1)
        ...
        for t in range(amount_of_timesteps):
            ...
            try:
                ...
                x_train_new = df_t.values.T[2:10].T[max_iterations:] - df_t.values.T[2:10].T[:max_iterations]
                x_train = np.vstack((x_train,x_train_new.flatten()))
                ...
                x_train_new = df_t.values.T[2:10].T[max_iterations:] - df_t.values.T[2:10].T[:max_iterations]
                x_train = np.append(x_train,x_train_new.flatten())
                current_timestep = df_t["Timestep"].max()
                angles = df_t.loc[df["Timestep"]==current_timestep]['angle'].values[0] #- df_t.loc[df["Timestep"]==(current_timestep-1)]['angle'].values[0]
    ...
    for file in test_files:
        ...
        for t in range(amount_of_timesteps):
            ...
            try:
                ...
                x_test_new = df_t.values.T[2:10].T[max_iterations:] - df_t.values.T[2:10].T[:max_iterations]
                x_test = np.vstack((x_test,x_test_new.flatten()))
                current_timestep = df_t["Timestep"].max()
                angles = df_t.loc[df["Timestep"]==current_timestep]['angle'].values[0] #- df_t.loc[df["Timestep"]==(current_timestep-1)]['angle'].values[0]
                #angles = df_t['angle'].values[-1] - df_t['angle'].values[-2]
